...
class IndexController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('frontend.welcome', compact('jurusanM','kegiatanM','slider','about','video','pengajar','berita','event','footer','top_students'));
    }
    ...
    public function profileSekolah()
    {
        ...
        $top_students = $this->getTopStudents();
        return view('frontend.content.profileSekolah', compact('profile','jurusanM','kegiatanM','pengajar','footer','top_students'));
    }
    ...
    public function visimisi()
    {
        ...
        $top_students = $this->getTopStudents();
        return view('frontend.content.visimisi', compact('visimisi','jurusanM','kegiatanM','pengajar','footer','top_students'));
    }

    private function getTopStudents()
    {
        $students = Mahasiswa::with(['penghargaans', 'sertifikats', 'perlombaans', 'portofolios'])
            ->where('status', 'Aktif')
            ->get();
        ...
        // Assign ranks and take the top 4
        $rank = 1;
        $prev_poin = null;
        $ranked_students = [];
        $i = 1;
        foreach ($sorted as $student) {
            // Only rank students with points > 0
            if ($student->total_poin <= 0) {
                continue;
            }
        ...
        return array_slice($ranked_students, 0, 4);
    }
}
